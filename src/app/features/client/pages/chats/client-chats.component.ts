    ...
    <div class="flex flex-col h-full">
      ...
      <div class="flex-1 overflow-y-auto">
        @if (loading()) {
          ...
          <app-empty-state icon="💬" title="Sin conversaciones" description="Cuando contactes un proveedor, aparecerá aquí."></app-empty-state>
        } @else {
          <div class="divide-y divide-surface-100">
            @for (conv of conversations(); track conv.id) {
              <a [routerLink]="['/client/chat', conv.id]" class="flex items-center gap-3 px-4 py-3.5 hover:bg-surface-50 transition-colors">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between">
                    <h3 class="font-semibold text-slate-800 truncate text-sm">{{ conv.display_name ?? 'Proveedor' }}</h3>
                    <span class="text-xs text-slate-400 shrink-0 ml-2">{{ conv.updated_at | date:'HH:mm' }}</span>
                  </div>
